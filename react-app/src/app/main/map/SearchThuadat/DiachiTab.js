...
function DiachiTab() {
    ...
    return (
        <div>
                    ...
                    <InputField name='diachi' label='Địa chỉ' InputProps={{
                        endAdornment: (
                            <InputAdornment position='end'>
                                {val.length > 1 ? (
                                    <IconButton edge='end' onClick={onReset}><CloseIcon/></IconButton>) : (
                                    <SearchIcon/>)}
                            </InputAdornment>
                        )
                    }}/>
            ...
            <Alert severity="info" className="mx-12 text-sm flex items-center">Bạn vui lòng nhập từ khóa (địa chỉ, vị
                trí, tọa độ) để tra cứu thửa đất</Alert>}
                ...
                <div className="border-t border-b">
                    <List className="divide-y p-0">
                        {_.map(autoSuggestData, (place, k) => {
                            return (
                                <ListItem key={k} button sx={{border: 'inherit'}}
                                          onClick={() => getByPlace(place.place_id)}>
